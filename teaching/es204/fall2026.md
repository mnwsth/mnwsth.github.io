...
Course content (CAT-SOOP): [Introduction to Digital Electronics](https://courses.madhava.org.in/digital-electronics-course/) · [Modules](https://courses.madhava.org.in/digital-electronics-course/modules). Google Classroom join code: **4ygrupb7**.
...
| Head TA | N J Vinay Chandra Singh | Use staff alias below |
| Head TA | Neal Apte Pineda | Use staff alias below |

**TAs:** Alaukik Gupta · Arjun K Reju · Arka Dutta · Durgesh Mishra · Jheel Jhala · Kolla Jaya Prakash · Nelson Singh Shagolsem · Punjaji Omkar Venkatrao · Saaransh Shandilya · Samiksha D Kamble · Sanjana Mehta · Syed Mohd Tashif

**Teams:** Teams of **3** (need a good reason for a different team size). Form teams by **end of Week 1** (default).

**Kits:** Every team receives one component kit (breadboards, wires, ICs, LEDs, DIP switches, etc.) — shared responsibility. Verify components against the inventory sheet when it is sent. Most contact time is spent building, not listening to lectures.
...
| Platform | Role |
|----------|------|
| **[CAT-SOOP](https://courses.madhava.org.in/digital-electronics-course/)** | Modules, exercises, projects |
| **Google Classroom** (join code **4ygrupb7**) | Announcements, discussion/help, and some submissions |

- **All announcements for the class will be made via Google Classroom.** Make sure you sign up using join code **4ygrupb7**.
...
### Exercises are individual (CAT-SOOP)

- **Every exercise** on CAT-SOOP is completed **individually**.
- Exception: **team video** uploads (proof of work) — see milestone demos below.
- Same modules for everyone; your answers / checkoffs are **yours**.
- Build with your team in lab; submit exercises under **your** login.
...
|-----------|--------|--------|
| Projects | **60%** | Team |
| Reflections | **10%** | Individual |
| Attendance | **10%** | Individual |
| Final exam | **20%** | Individual |

#### Projects (60%) — working hardware + proof of work

Suggested internal split (sums to 60%; may be adjusted):
...
Attendance is worth **10%** of the course grade. Students are **expected to attend all tutorials, lectures, and labs**.

- You may take up to **2 absences** that do **not** count toward the attendance formula (e.g. emergencies, interviews). For each of these, you must still email [`es204-staff@googlegroups.com`](mailto:es204-staff@googlegroups.com) declaring the reason for your absence.
- Let **n** be the number of absences **beyond** those two allowed absences.
- Your attendance marks (out of 10) are given by **10 − n² / 4** (floored at 0 if the formula would go negative).

| Counted absences *n* | Attendance marks |
|----------------------|------------------|
| 0 | 10 |
| 1 | 9.75 |
| 2 | 9 |
...
### Presence and milestone demos

Students are **expected to attend all tutorials, lectures, and labs**. Contact hours exist so you can build with staff present and complete **milestone demos**. Attendance contributes **10%** of the course grade (see formula above).

All groups are expected to **carry the designated kits** with them during each of the contact-hour slots. The instructors and core staff **reserve the right to cold-call any group** and check on progress during any lecture, tutorial, or lab session.
...
**Proof-of-work videos (team)**

- Each milestone / project video is a **team** deliverable.
- The **team leader** uploads the video for the team.
- One upload per team — not one per member.

**Team-member contribution (individual)**

- Every team upload also needs a **contribution** write-up from each member.
- That contribution is submitted **individually**.
- Each member writes about the contributions of the other team members.
- Exact submission modality (how / where) will be announced on Google Classroom.
...
- **1 day late:** −25% of the assignment’s value.
- **Each successive day late:** another −25%.
- Once the cumulative late penalty reaches **−75%** (i.e. the submission is worth at most **25%**), further delay does not reduce credit below 25%. You may still submit anytime until the **end of the course**, but you will receive at most **25%** of the assignment’s value.
- Google Forms and related submission links remain open until the end of the course.
...
- Teams of **3** (need a good reason for a different team size).
...
- Kits are issued **one per team**. All members share responsibility for the kit.
- Verify components in the kit against the inventory sheet when it is sent. Report missing or damaged parts to TAs promptly.
...
- The **team leader** uploads **one** video per team via CAT-SOOP and/or Google Classroom, as specified for that assignment. Never by email.
- Videos are used for grading; do not post course videos publicly unless permitted.
...
- Join Google Classroom (code **4ygrupb7**); form a team of **3**; receive kit and check inventory.
- Read policies on this website; complete Pre-requisite / Module 0 on CAT-SOOP (LEDs and Basic Circuits).
- **Milestone:** live demo of DIP-controlled 7-segment setup, plus team video and individual contribution write-ups.
